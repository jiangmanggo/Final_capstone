<?php
    ...
    if (isset($_SESSION['id'])) {
        ...
        $sql = "SELECT `order`.`product_name`, `farmer`.`fname`, `farmer`.`lname`, `farmer`.`association`, MONTH(`order`.`date_ordered`) AS order_month, SUM(`order`.`quantity`) AS product_count FROM `order` INNER JOIN farmer ON `order`.`seller_id` = `farmer`.`seller_id` WHERE `order`.`status` = 'received' AND MONTH(`order`.`date_ordered`) = MONTH(CURDATE()) AND YEAR(`order`.`date_ordered`) = YEAR(CURDATE()) GROUP BY `order`.`product_name`";
...
<html lang="en">
    <head>
        ...
        <!-- <link href="../css/superA_sales.css" rel="stylesheet"> -->
        <title>BCAAOMS</title>
        <style>
    ...
    <body>
        <?php
            ...
            $sqlSales = "SELECT `order`.`seller_id`, COUNT(`order`.`order_id`) AS total_Sales FROM `order` WHERE `order`.`status` = 'received' AND MONTH(`order`.`date_ordered`) = MONTH(CURDATE()) AND YEAR(`order`.`date_ordered`) = YEAR(CURDATE());";
            ...
            $sqlOrders = "SELECT COUNT(*) AS total_Orders FROM ( SELECT `order`.`reference_no` FROM `order` WHERE `order`.`status` = 'received' AND MONTH(`order`.`date_ordered`) = MONTH(CURDATE()) AND YEAR(`order`.`date_ordered`) = YEAR(CURDATE()) GROUP BY `order`.`reference_no` ) AS subquery;";
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    ...
                                    var farmerName = farmerNames[context.dataIndex]; 
        ...
        <script>
            ...
            Use the PHP variables for the totalSales and indicatorWidth
            const totalSales = <?php echo $totalSales; ?>;
            const indicatorWidth = (totalSales / maxValue) * 100;

            const indicatorElement = document.getElementById('indicator');
            const totalValueElement = document.getElementById('totalValue');
            indicatorElement.style.width = `${indicatorWidth}%`;
            totalValueElement.textContent = totalSales;
