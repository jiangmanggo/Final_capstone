<?php
    ...
    if (isset($_SESSION['id'])) {
        ...
        $sql = "SELECT `order`.`product_name`, `farmer`.`fname`, `farmer`.`lname`, MONTH(`order`.`date_ordered`) AS order_month, SUM(`order`.`quantity`) AS product_count FROM `order` INNER JOIN farmer ON `order`.`seller_id` = `farmer`.`seller_id` INNER JOIN `association_chairman` ON `farmer`.`association` = `association_chairman`.`association` WHERE `order`.`status` = 'received' AND `association_chairman`.`admin_id` = '$admin_id' AND MONTH(`order`.`date_ordered`) = MONTH(CURDATE()) AND YEAR(`order`.`date_ordered`) = YEAR(CURDATE()) GROUP BY `order`.`product_name`";
        ...
        if ($result) {
            while ($row = $result->fetch_assoc()) {
                ...
                $farmerNames[] = $row['fname'] . ' ' . $row['lname'];
            }
        }else {
...
<html lang="en">
    <head>
        ...
        <!-- <link href="../css/admin_sales.css" rel="stylesheet"> -->
        <title>BCAAOMS</title>
        <style>
    ...
    <body>       
        <?php
            ...
            if (isset($_SESSION['id'])) {
                ...
                $sqlSales = "SELECT `order`.`seller_id`, COUNT(`order`.`order_id`) AS total_Sales FROM `order` INNER JOIN `farmer` ON `order`.`seller_id` = `farmer`.`seller_id` INNER JOIN `association_chairman` ON `farmer`.`association` = `association_chairman`.`association` WHERE `order`.`status` = 'received' AND `association_chairman`.`admin_id` = '$admin_id' AND MONTH(`order`.`date_ordered`) = MONTH(CURDATE()) AND YEAR(`order`.`date_ordered`) = YEAR(CURDATE());";
                ...
                $sqlOrders = "SELECT COUNT(*) AS total_Orders FROM ( SELECT `order`.`reference_no` FROM `order`INNER JOIN `farmer` ON `order`.`seller_id` = `farmer`.`seller_id` INNER JOIN `association_chairman` ON `farmer`.`association` = `association_chairman`.`association` WHERE `order`.`status` = 'received' AND `association_chairman`.`admin_id` = '$admin_id' AND MONTH(`order`.`date_ordered`) = MONTH(CURDATE()) AND YEAR(`order`.`date_ordered`) = YEAR(CURDATE()) GROUP BY `order`.`reference_no` ) AS subquery;";
                    ...
                    <div class="col-12 col-lg-6">
                        <div class="data-graph">
                            <div class="d-flex flex-column btn-with-graph">
                                ...
                                <button class="btn-sales focus" id="monthly"><a href="sales.php" style="color: black; text-decoration: none;">MONTHLY</a></button>
        ...
        <script>
            ...
            var farmerNames = <?php echo json_encode($farmerNames); ?>;

            var backgroundColors = productNames.map(function() {
                return getRandomColor(); 
            });

            var hoverColors = productNames.map(function() {
                return getRandomColor(); 
            });
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    ...
                                    var productCount = context.dataset.data[context.dataIndex];
                                    var farmerName = context.dataset.farmerNames[context.dataIndex];
                                    label += ': ' + productCount + ' Sales\nby ' + farmerName;
                                    return label;
                                }
                            }
                        }
