<?php
    ...
    if (isset($_GET['farmer_id'])) {
        ...
        $query = "SELECT `product_list`.`product_id`, `product_list`.`product_category`, `product_list`.`product_name`, `product_list`.`price`, `product_list`.`date_posted`, `product_list`.`product_id` FROM `product_list` WHERE `product_list`.`seller_id` = '$seller_id'";
...
<html lang="en">
    <head>
        ...
        <link href="../css/superA_view_products.css" rel="stylesheet">
        <title>BCAAOMS</title>
    </head>
    <body>
                                ...
                                <table id="farmers" style="width:100%" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><a href="delete_accounts/delete_products.php?id=<?php echo $farmerrow['product_id']?>" onclick="return confirm('Are you sure you want to delete this product?');"><button class='btn btn-danger'><i class="fas fa-trash"></i></button></a></td>
                                        </tr>
                                        <?php
                                            }
                                        ?>
