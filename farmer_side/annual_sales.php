<?php
    ...
    $sql = "SELECT MONTH(`order`.`date_ordered`) AS order_month, SUM(`order`.`quantity`) AS product_count FROM `order` INNER JOIN farmer ON `order`.`seller_id` = `farmer`.`seller_id` WHERE `order`.`status` = 'received' AND `farmer`.`seller_id` = '$seller_id' AND YEAR(`order`.`date_ordered`) = YEAR(CURDATE()) GROUP BY MONTH(`order`.`date_ordered`)";
...
<html lang="en">
    <head>
        ...
        <!-- <link href="../css/farmer_annual_sales.css" rel="stylesheet"> -->
        <title>BCAAOMS</title>
        <style>
    ...
    <body>
        <?php
            ...
            if (isset($_SESSION['id'])) {
                ...
                $sqlSales = "SELECT `order`.`seller_id`, COUNT(`order`.`order_id`) AS total_Sales FROM `order` INNER JOIN `farmer` ON `order`.`seller_id` = `farmer`.`seller_id` WHERE `order`.`status` = 'received' AND YEAR(`order`.`date_ordered`) = YEAR(CURDATE());";
                ...
                $sqlOrders = "SELECT COUNT(*) AS total_Orders FROM ( SELECT `order`.`reference_no` FROM `order`INNER JOIN `farmer` ON `order`.`seller_id` = `farmer`.`seller_id` WHERE `order`.`status` = 'received' AND YEAR(`order`.`date_ordered`) = YEAR(CURDATE()) GROUP BY `order`.`reference_no` ) AS subquery;";
