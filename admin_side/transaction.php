<?php
    ...
    if (isset($_SESSION['id'])) {
        ...
        $query = "SELECT `order`.`reference_no`, `order`.`date_ordered`, `order`.`status`, `order`.`seller_id`, `farmer`.`fname` AS `farmer_name`, `order`.`fname` AS `buyer_name`, `order`.`address` FROM `order` INNER JOIN `farmer` ON `farmer`.`seller_id` = `order`.`seller_id` INNER JOIN `association_chairman` ON `farmer`.`association` = `association_chairman`.`association` WHERE `association_chairman`.`admin_id` = '$admin_id' AND `order`.`status` IN ('$statusString') GROUP BY `order`.`reference_no` ORDER BY `order`.`date_ordered` DESC";
...
<head>
    ...
    <link href="../css/admin_transaction.css" rel="stylesheet">
    <title>BCAAOMS</title>
</head>
<body>
