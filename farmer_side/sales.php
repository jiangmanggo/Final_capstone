    ...
    <body>
        <?php
            ...
            if (isset($_SESSION['id'])) {
                ...
                $sqlSales = "SELECT `order`.`seller_id`, COUNT(`order`.`order_id`) AS total_Sales FROM `order` INNER JOIN `farmer` ON `order`.`seller_id` = `farmer`.`seller_id` WHERE `order`.`status` = 'received' AND `farmer`.`seller_id` = '$seller_id' AND MONTH(`order`.`date_ordered`) = MONTH(CURDATE()) AND YEAR(`order`.`date_ordered`) = YEAR(CURDATE())";
                ...
                $sqlOrders = "SELECT COUNT(*) AS total_Orders FROM ( SELECT `order`.`reference_no` FROM `order`INNER JOIN `farmer` ON `order`.`seller_id` = `farmer`.`seller_id` WHERE `order`.`status` = 'received' AND `farmer`.`seller_id` = '$seller_id' AND MONTH(`order`.`date_ordered`) = MONTH(CURDATE()) AND YEAR(`order`.`date_ordered`) = YEAR(CURDATE()) GROUP BY `order`.`reference_no` ) AS subquery;";
